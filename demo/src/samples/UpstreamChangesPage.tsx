import { ControlledFieldManager, type FieldValues, InputField, useFieldState } from '@iwsio/forms'
	...
	return (
		...
		>
			...
			<div className="flex gap-2">
				<button type="reset" className="btn btn-info" onClick={() => refetch()}>
					Re-fetch
				</button>
				<button
					type="reset"
				...
				*/}
				<button
					type="submit"
					disabled={isFormBusy}
					className={`btn ${success ? 'btn-success' : 'btn-primary'} ${isFormBusy ? 'btn-disabled' : ''}`}
				>
					Submit
				</button>
			</div>
			<p>
				Try clicking <strong>Reset</strong> to reset the form. Then <strong>Submit</strong> will show validation errors.
				Then try clicking the <strong>Re-fetch</strong> button to fetch new data from the server and reset the field
				validation.
			</p>
		</ControlledFieldManager>
	)
